...
function endGame(win=false){running=false;setTimeout(()=>{alert(win?`¡Bahía a la vista! Rescataste a los ${goal} patitos y sumaste ${Math.floor(score)} puntos.`:`Pipa necesita una vuelta más. Rescataste ${rescued} patitos.`);closeModal($('#game'))},100)}
